...
SlideBar* SlideBar::createBar(int size, Point poing, int number)
{
	SlideBar* layer = SlideBar::create();
	if (layer)
	{
		layer->barInit(size, poing, number);
		return layer;
	}
...
bool SlideBar::onTouchBegan(Touch *touch, Event *unused_event)
{
...
void SlideBar::onTouchMoved(Touch *touch, Event *unused_event)
{
	if (!isTouched)
	{
		return;
...
void SlideBar::onTouchEnded(Touch *touch, Event *unused_event)
{
	isTouched = false;
}